...
def var_coll(WTc,OSSc,X,Y,UL,Cables,T,feasibility):
...
#%% WTs
    final_col=np.kron(np.ones((T,1)),np.arange(0,UL,1).reshape(UL,1))                 
    for i in range(OSSc+1,OSSc+WTc+1):
        ...
        final_matrix=np.repeat(var_output_aux[:T,:],UL,axis=0)
        var_output=np.vstack((var_output,np.hstack((final_matrix,final_col))))                            
...
def objective_function(Bil,WTn):
    len_wt_costs=int(WTn*(WTn-1)/2)
    power_coefficients=np.zeros((len_wt_costs,3))
    cont=0
    for i in range(WTn):
        for j in range(i+1,WTn):
            power_coefficients[cont,0],power_coefficients[cont,1],power_coefficients[cont,2]=i,j,Bil[i,j]+Bil[j,i]
            cont+=1
...
if __name__ == "__main__":
    ...
    T=5
    feasibility=0
    var_output,objective,cable_selected=var_coll(WTc,OSSc,X,Y,UL,Cables,T,feasibility)
